$(function () {
    ...
    $("table#pages input.text-box").blur(function () {
        ...
        var newBrandName = $this.val();
        var id = $this.parent().parent().parent().parent().parent().attr("id").substring(3);
        var url = "/admin/shop/RenameBrand";

        if (newBrandName.length < 3) {
            alert("Brand name must be at least 3 characters long.");
            $this.attr("readonly", true);
            return false;
        }

        $.post(url, { newBrandName: newBrandName, id: id }, function (data) {
            var response = data.trim();
            ...
            if (response == "titletaken") {
                ...
                ajaxdiv.html("<div class='alert alert-success'>The brand name has been changed!</div>").show();
            }
    ...
    //        var url = "/Admin/Shop/ReorderBrands";
